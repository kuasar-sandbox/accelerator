...
# Hash source contents independently of archive compression and metadata.
# GitHub-generated tag archives can be regenerated byte-for-byte differently
# while still containing the same source tree.
source_tree_sha256() {
    local root=$1 path mode digest
    [ -d "$root" ] || die "source tree not found: $root"
    require_cmd find sort stat sha256sum readlink
    ...
    (
        ...
        while IFS= read -r -d '' path; do
            mode="$(stat -c %a -- "$path")"
            if [ -L "$path" ]; then
                printf 'link\0%s\0%s\0' "$mode" "${path#./}"
                readlink -z -- "$path"
            elif [ -d "$path" ]; then
                printf 'dir\0%s\0%s\0' "$mode" "${path#./}"
            elif [ -f "$path" ]; then
                digest="$(sha256sum -- "$path")"
                printf 'file\0%s\0%s\0%s\0' "$mode" "${path#./}" "${digest%% *}"
            fi
        done < <(find . -mindepth 1 ! -path './.extracted' -print0 | LC_ALL=C sort -z)
