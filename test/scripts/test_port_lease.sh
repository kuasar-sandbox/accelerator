...
source "$ROOT/test/e2e/lib/port_lease.sh"
...
if grep -qvE '^[0-9]+$' "$TMPDIR/allocated"; then
    echo "port lease source regression: allocator returned a non-numeric value" >&2
    exit 1
fi
...
if [ "$allocated" -ne "$count" ] || [ "$unique" -ne "$count" ] || [ "$leased" -ne "$count" ]; then
    echo "port lease source regression: allocated=$allocated unique=$unique leased=$leased expected=$count" >&2
    exit 1
fi

echo "port lease source regression: PASS ($rapid_count rapid + $concurrent_count concurrent unique leases)"
