...
Thank you for contributing to `kuasar-sandbox/accelerator`.
...
```bash
git clone git@github.com:<your-login>/accelerator.git
cd accelerator
git remote add upstream git@github.com:kuasar-sandbox/accelerator.git
```
...
Open a pull request to the appropriate target: `main` for current development, or a supported `release/vMAJOR.MINOR.x` maintenance branch. For example, use `<your-login>/accelerator:feat/short-description` → `kuasar-sandbox/accelerator:main`. Enable **Allow edits from maintainers** when appropriate.
