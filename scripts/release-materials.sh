...
release_materials_require_rocksdb_notices() {
  local root="$1" name
  for name in AUTHORS COPYING LICENSE.Apache LICENSE.leveldb; do
    [ -s "$root/share/licenses/accelerator/rocksdb/$name" ] \
      || fail "required RocksDB notice is missing: $name"
  done
}
...
release_materials_go_payload_allowed() {
  case "$1:$2" in
    accelerator:bin/manifest-ctl|accelerator:bin/store-ctl|accelerator:bin/cache-ctl ) return 0 ;;
...
release_materials_finish() {
  ...
  while IFS=$'\t' read -r module version checksum; do
    [ -n "$module" ] || continue
    # This component has no separately recorded internal Go dependency.
...
release_materials_require_source() {
  ...
  case "$name" in
    ...
    "$unit") label=project ;;
    rocksdb|rocksdb-static-library) label=rocksdb ;;
...
release_materials_validate() {
  ...
  while IFS=$'\t' read -r module version checksum || [ -n "$module" ]; do
    # No namespace-wide exemption from module notice checks.
    [[ "$checksum" =~ ^h1:[A-Za-z0-9+/]{43}=$ ]] \
      || fail "third-party Go source requires a module checksum: $module@$version"
