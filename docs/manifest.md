...
- [prefetch concurrency](prefetch.md): overlapping prefetch Gets, yield to on-demand, and the `maxPrefetchGets` cap.
- [prefetch concurrency](prefetch.md): concurrent prefetch Gets, yield to on-demand UFFD faults.
- [cache](cache.md): optional acceleration for reads; cache-ctl can use the same store client as its origin.
- [prefetch concurrency](prefetch.md): overlapping prefetch Gets, yield to on-demand UFFD faults.
