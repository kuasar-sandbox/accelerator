...
| `pkg/tar` | **tar 提取面**:从任意 tar 流取文件(`Rule`+`Extract`),纯 Go 单遍流式,零值是数据不造洞;稀疏成员经 `Options.Reopen` 按序数二次定位**全路径声明洞精确**(stdlib 洞图未导出,issue 22735),`Options.Dense` 显式回 stdlib 致密;`ExtractFile` 单成员精确还原 | stdlib + `sandbox-accelerator/pkg/{sparse,tarstream}` |
| `cmd/flatten-ctl` | CLI:`export`(registry/docker-archive/rootfs 目录三源,产物=tarstream 镜像工件 .img)/`info`/`cache`/`config`/`tar`/`mountpoint`;`--upload` 经 `sandbox-accelerator/pkg/manifest` ingest 进 store | accelerator SDK、`pkg/remote`、`pkg/tar` |
