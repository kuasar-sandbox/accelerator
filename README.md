...
| `pkg/tar` | **tar 提取面**:从任意 tar 流取文件(`Rule`+`Extract`),纯 Go 单遍流式,常规文件致密落盘(零值是数据、不造洞);`ExtractFile` 对单成员按声明洞精确还原;重命名与 stdio 规则 | stdlib + `sandbox-accelerator/pkg/{sparse,tarstream}` |
