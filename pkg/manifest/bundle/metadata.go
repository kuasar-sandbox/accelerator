...
func readMetadataPrefix(source io.ReaderAt, size int64) (Metadata, int64, int, error) {
	if source == nil {
		return Metadata{}, 0, 0, readerr.Mark(fmt.Errorf("manifest bundle: metadata source is required"), false)
	}
	...
	if first.name == refsName {
		if first.size == 0 {
			return Metadata{}, 0, 0, readerr.Mark(fmt.Errorf("manifest bundle: refs payload must be non-empty"), false)
		}
		if first.size > maxRefsPayload {
			return Metadata{}, 0, 0, readerr.Mark(fmt.Errorf("manifest bundle: refs payload exceeds %d bytes", maxRefsPayload), false)
		}
		...
		if crc32.ChecksumIEEE(payload) != first.crc {
			return Metadata{}, 0, 0, readerr.Mark(fmt.Errorf("manifest bundle: %s CRC mismatch", refsName), false)
		}
	...
	if strings.HasPrefix(admissionEntry.name, legacyAdmissionPrefix) {
		return Metadata{}, 0, 0, readerr.Mark(fmt.Errorf("manifest bundle: legacy admission entry %q is not allowed", admissionEntry.name), false)
	}
	if !strings.HasPrefix(admissionEntry.name, admissionPrefix) {
		return Metadata{}, 0, 0, readerr.Mark(fmt.Errorf("manifest bundle: metadata prefix expected admission, got %q", admissionEntry.name), false)
	}
	if admissionEntry.size != 0 || admissionEntry.crc != crc32.ChecksumIEEE(nil) {
		return Metadata{}, 0, 0, readerr.Mark(fmt.Errorf("manifest bundle: admission payload must be empty"), false)
	}
	...
	if metadata.admission.Salt != wantSalt {
		return Metadata{}, 0, 0, readerr.Mark(fmt.Errorf("manifest bundle: admission salt is not canonical for generation %q", metadata.admission.Generation), false)
	}
...
func readPrefixEntry(source io.ReaderAt, archiveSize, offset int64) (prefixEntry, error) {
	...
	if archiveSize < 0 || offset < 0 || fixedSize > archiveSize-offset {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: truncated metadata local header at offset %d", offset), false)
	}
	...
	if !bytesEqual4(fixed[:4], zipLocalHeaderMagic) {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: invalid metadata local-header magic at offset %d", offset), false)
	}
	if version := binary.LittleEndian.Uint16(fixed[4:6]); version != 45 {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: metadata local reader version %d is not canonical", version), false)
	}
	if flags := binary.LittleEndian.Uint16(fixed[6:8]); flags != 0 {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: metadata local flags 0x%x are not allowed", flags), false)
	}
	if method := binary.LittleEndian.Uint16(fixed[8:10]); method != zip.Store {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: metadata local method %d, want Store", method), false)
	}
	if binary.LittleEndian.Uint16(fixed[10:12]) != 0 || binary.LittleEndian.Uint16(fixed[12:14]) != 0 {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: metadata local timestamp is not canonical"), false)
	}
	...
	if compressed != uncompressed || uint64(uncompressed) > math.MaxInt {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: metadata Store sizes are invalid"), false)
	}
	nameSize := int(binary.LittleEndian.Uint16(fixed[26:28]))
	if nameSize == 0 {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: metadata entry name is empty"), false)
	}
	if binary.LittleEndian.Uint16(fixed[28:30]) != 0 {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: metadata local extra field is not allowed"), false)
	}
	nameOffset := offset + fixedSize
	if int64(nameSize) > archiveSize-nameOffset {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: truncated metadata entry name"), false)
	}
	...
	if int64(size) > archiveSize-dataOffset {
		return prefixEntry{}, readerr.Mark(fmt.Errorf("manifest bundle: truncated metadata entry %q", string(nameBytes)), false)
	}
	return prefixEntry{
